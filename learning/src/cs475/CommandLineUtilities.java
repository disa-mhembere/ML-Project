...
public class CommandLineUtilities
{
  ...
  /**
   * This uses the apache Jakarta CLI to parse the command line. A single static
   * instance of this class exists for global access by all parts of the
   * program. To use this class, a list of options must be specified and passed
   * to this method. Mandatory arguments should be encoded as strings.
   * 
   * @param args
   *          The command line received by main
   * @param manditory_args
   *          A list of strings that contain the names of mandatory arguments.
   * @param specified_options
   *          A list of options to use for this program.
   */
  public static void initCommandLineParameters(String[] args,
      LinkedList<Option> specified_options, String[] manditory_args)
  {
    Options options = new Options();
    if (specified_options != null) for (Option option : specified_options)
    ...
    try
    {
      CommandLineUtilities._command_line = command_line_parser.parse(options,
          args);
    ...
    {
      String parameter_file = CommandLineUtilities
          .getOptionValue("parameter_file");
  ...
  public static boolean hasArg(String option)
  {
    if (CommandLineUtilities._command_line.hasOption(option)
        || CommandLineUtilities._properties.containsKey(option)) return true;
    return false;
  }

  /**
   * This method is used when there is one argument for the option presented as
   * a ";" separated list.
   * 
   * @param option
   * @return
  ...
  public static String[] getOptionValues(String option)
  {
    ...
    if (CommandLineUtilities._command_line.hasOption(option))
      arguments_to_parse = CommandLineUtilities._command_line
          .getOptionValue(option);
    if (CommandLineUtilities._properties.containsKey(option))
    {
      arguments_to_parse = (String) CommandLineUtilities._properties
          .getProperty(option);
    }
